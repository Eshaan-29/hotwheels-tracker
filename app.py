...
def check_stock():
    try:
        ...
        for url in URLS:
            ...
            # 🛑 THE FIX: Look at the main product wrapper instead of just the text
            product_blocks = soup.find_all('div', class_='list_block') 
            
            for block in product_blocks:
                # If FirstCry marked it as out of stock, skip this car immediately!
                if block.get('data-outstock') == 'true':
                    continue 
                
                # If it IS in stock, grab the title
                        ...
                        for target in TARGET_CARS:
                            if target in title_lower:
                                if full_title not in already_alerted:
                                    # Formatted alert to emphasize it is actually buyable
        ...
        if alerts_sent > 0:
            return f"Checked both pages! Found {alerts_sent} IN STOCK items.", 200
        else:
            return "Checked both pages safely. No new IN STOCK items found.", 200
